...
NAN_METHOD(OpenFunction)
{
	...
	if (m_pKinectSensor)
	{
		// Does some of the same functions in FaceBasics, but in a different order
		// Only opens the coordinate mapper and the kinect sensor immediately
		if (SUCCEEDED(hr))
		{
...
/////// LONG EXPOSURE INFRARED FUNCTIONS
...
// This is for the relative distance to floor stuff
...
// Multi source continued - body indices
...
// NAN_MODULE_INIT is the initialization logic for the module
...
// Create the module with the name "kinect2", and use the 'Init' function to initialize
